<%@ Page Title="" Language="C#" MasterPageFile="~/Site1.Master" AutoEventWireup="true" CodeBehind="Producto.aspx.cs" Inherits="McSoftware.View.Producto" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="MainFooter" runat="server">
    ...
    <script>
        ...
        function CBO_LIST_GENERAL() {
            //cargado Button

            var data = JSON.stringify({});
            $.ajax({
                type: "POST",
                url: "Producto.aspx/LIST_BUTTON_CURSOS",
                data: data,
                ...
                success: function (data) {
                    ...
                    if (datos.length > 0) {
                        $(datos).each(function () {
                            ...
                            var li_div_div = document.createElement('div');//li
                            li_div_div.setAttribute("class", "text-left col-sm-12 col-md-12 col-lg-12");
                            li_div_div.setAttribute("style", "padding-bottom: 15px; white-space: normal;");


                            //li_div.id = this.nidCuestionario;
                            //li_div.appendChild(document.createTextNode(this.sDescripcion));

                            var li_div = document.createElement('button');//li
                            li_div.setAttribute("type", "button");
                            li_div.setAttribute("style", "white-space: normal;");
                            li_div.setAttribute("class", "btn btn-primary");
                            li_div.id = this.IdMateria;
                            // Evento:
                            li_div.onclick = Cargado_Cuestionario_Curso;

                            li_div.appendChild(document.createTextNode(this.Nombre));



                            li_div_div.appendChild(li_div);


                            $("#Botones").append(li_div_div);



                            //var widget = document.getElementById("Botones");
